...
import edu.boun.edgecloudsim.applications.sample_app6.SampleScenarioFactory;
...
public class MainApp {
	...
	public static void main(String[] args) {
		...
		if (args.length == 5) {
			...
			mobileDevicesFile = "scripts/sample_app6/config/mobile_devices_new100.xml";
			outputFolder = "sim_results/ite" + iterationNumber;
		}

		// load settings from configuration file
		SimSettings SS = SimSettings.getInstance();
		if (SS.initialize(configFile, edgeDevicesFile, applicationsFile, mobileDevicesFile) == false) {
		...
		for (int j = SS.getMinNumOfMobileDev(); j <= SS.getMaxNumOfMobileDev(); j += SS.getMobileDevCounterSize()) {
			for (int k = 0; k < SS.getSimulationScenarios().length; k++) {
				for (int i = 0; i < SS.getOrchestratorPolicies().length; i++) {
					...
					SimLogger.printLine("Scenario: " + simScenario + " - Policy: " + orchestratorPolicy
							+ " - #iteration: " + iterationNumber);
					SimLogger.printLine("Duration: " + SS.getSimulationTime() / 60 + " min (warm up period: "
							+ SS.getWarmUpPeriod() / 60 + " min) - #devices: " + j);
					SimLogger.getInstance().simStarted(outputFolder,
							"SIMRESULT_" + simScenario + "_" + orchestratorPolicy + "_" + j + "DEVICES");
		...
		SimLogger.printLine("Simulation finished at " + now + ". It took "
