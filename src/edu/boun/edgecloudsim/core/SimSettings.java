...
public class SimSettings {
	...
	public static enum VM_TYPES {
		MOBILE_VM, EDGE_VM, CLOUD_VM
	}

	// enumarations for the VM types
	public static enum NETWORK_DELAY_TYPES {
		WLAN_DELAY, MAN_DELAY, WAN_DELAY
	}
	 ...
	 * @return
	 */
	public boolean initialize(final String propertiesFile, final String edgeDevicesFile, final String applicationsFile,
			final String mobileDevicesFile) {
		...
		try {
			...
			INTERVAL_TO_GET_VM_LOCATION_LOG = (double) 60
					* Double.parseDouble(prop.getProperty("vm_location_check_interval")); // seconds
			...
			final double place1_mean_waiting_time = Double
					.parseDouble(prop.getProperty("attractiveness_L1_mean_waiting_time"));
			final double place2_mean_waiting_time = Double
					.parseDouble(prop.getProperty("attractiveness_L2_mean_waiting_time"));
			final double place3_mean_waiting_time = Double
					.parseDouble(prop.getProperty("attractiveness_L3_mean_waiting_time"));

			// mean waiting time (minute)
			mobilityLookUpTable = new double[] { place1_mean_waiting_time, // ATTRACTIVENESS_L1
	...
	/**
	 * returns VM utilization log collection interval (in seconds unit) from
	 * properties file
	 */
	...
	/**
	 * returns VM location log collection interval (in seconds unit) from properties
	 * file
	 */
	...
	/**
	 * returns the number of increase on mobile devices while iterating from min to
	 * max mobile device
	 */
	...
	/**
	 * returns mobility characteristic within an array the result includes mean
	 * waiting time (minute) or each place type
	 */
	...
	/**
	 * returns application characteristic within two dimensional array the result
	 * includes the following values for each application type [0] usage percentage
	 * (%) [1] prob. of selecting cloud (%) [2] poisson mean (sec) [3] active period
	 * (sec) [4] idle period (sec) [5] avg data upload (KB) [6] avg data download
	 * (KB) [7] avg task length (MI) [8] required # of cores [9] vm utilization on
	 * edge (%) [10] vm utilization on cloud (%) [11] vm utilization on mobile (%)
	 * [12] delay sensitivity [0-1]
	 */
	...
	private void parseApplicatinosXML(final String filePath) {
		...
		try {
			...
			for (int i = 0; i < appList.getLength(); i++) {
				...
				final double usage_percentage = Double
						.parseDouble(appElement.getElementsByTagName("usage_percentage").item(0).getTextContent());
				final double prob_cloud_selection = Double
						.parseDouble(appElement.getElementsByTagName("prob_cloud_selection").item(0).getTextContent());
				final double poisson_interarrival = Double
						.parseDouble(appElement.getElementsByTagName("poisson_interarrival").item(0).getTextContent());
				final double active_period = Double
						.parseDouble(appElement.getElementsByTagName("active_period").item(0).getTextContent());
				final double idle_period = Double
						.parseDouble(appElement.getElementsByTagName("idle_period").item(0).getTextContent());
				final double data_upload = Double
						.parseDouble(appElement.getElementsByTagName("data_upload").item(0).getTextContent());
				final double data_download = Double
						.parseDouble(appElement.getElementsByTagName("data_download").item(0).getTextContent());
				final double task_length = Double
						.parseDouble(appElement.getElementsByTagName("task_length").item(0).getTextContent());
				final double required_core = Double
						.parseDouble(appElement.getElementsByTagName("required_core").item(0).getTextContent());
				final double vm_utilization_on_edge = Double.parseDouble(
						appElement.getElementsByTagName("vm_utilization_on_edge").item(0).getTextContent());
				final double vm_utilization_on_cloud = Double.parseDouble(
						appElement.getElementsByTagName("vm_utilization_on_cloud").item(0).getTextContent());
				final double vm_utilization_on_mobile = Double.parseDouble(
						appElement.getElementsByTagName("vm_utilization_on_mobile").item(0).getTextContent());
				final double delay_sensitivity = Double
						.parseDouble(appElement.getElementsByTagName("delay_sensitivity").item(0).getTextContent());
